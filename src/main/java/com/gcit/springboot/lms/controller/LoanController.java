...
import com.gcit.springboot.lms.dao.LoanDAO;
...
public class LoanController {
	...
	public Loan getLoan(@PathVariable int cardNo, @PathVariable int branchId, @PathVariable int bookId, HttpServletResponse response ) throws IOException {
		...
		if (loan == null) 
			response.sendError(404, "Invalid composite keys. Loan entry does not exist in the database.");
		return loan;
	}
...
//	@RequestMapping(value = "/loans/borrowers/{cardNo}/branches/{branchId}/books/{bookId}/date/{dateIn}", method = RequestMethod.PUT)
//	public void updateLoan(@RequestBody @Valid Loan loan, @PathVariable int cardNo, @PathVariable int branchId, @PathVariable int bookId, @PathVariable String dateIn , HttpServletResponse response) {
//		logger.info("Updating loan with new due date: " + loan.getDueDate());
//		Timestamp di = new Timestamp(Long.parseLong(dateIn));
//		int i = loanDao.updateLoan(cardNo, branchId, bookId, di, loan.getDueDate());
//		logger.info(i + " loan updated with new due date: " + loan.getDueDate());
//		response.setHeader("location","/loans/borrowers/"+ cardNo + "/branches/" + branchId + "/books/" + bookId + "/date/" + dateIn);
//	}
}
